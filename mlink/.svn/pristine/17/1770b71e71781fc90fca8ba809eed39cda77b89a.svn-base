...
$file = array();
$api_url = 'http://api.mgsvc.com/?act=mlink.get_transaction&date=[datetime]';
$content = '';
...
foreach($date_range as $date){
	...
	$http_data['file_temp'] = DATA_ROOT.'bdgtradelog/transaction_'.str_replace('-','',$date).'.dat';
        _http($url,$http_data);

	$sql = 'DELETE FROM rpt_transaction_unique_inner WHERE CreatedDate = "'.$date.'" AND Source = "bdg"';
        $_db->query($sql);
