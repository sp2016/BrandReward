...
class Payments extends LibFactory{
    ...
    function getPayments($opts,$page=1,$page_size=50){
        ...
        if(isset($opts['ptype']) && !empty($opts['ptype'])){
            $tag = $opts['ptype'] == 'mk' ? true : false;
            $where_arr[] = mk_publisher_where($tag);
        }
    ...
    function nextPaymentTotal($opts){
        ...
        if(isset($opts['ptype']) && !empty($opts['ptype'])){
            $tag = $opts['ptype'] == 'mk' ? true : false;
            $where_aa .= " AND " . mk_publisher_where($tag);
        }
        ...
        if(isset($opts['hasbank']) && !empty($opts['hasbank'])){
            if(strtolower($opts['hasbank']) == 'yes')
                $where_cc_arr[] = "cc.`AccountNumber` != ''";
            else
                $where_cc_arr[] = "cc.`AccountNumber` = ''";
        }

        $where_cc_arr[] = "cc.PaymentStatus = 'active'";
        $where_cc = empty($where_cc_arr)?'':(' WHERE '.join(' AND ',$where_cc_arr));
