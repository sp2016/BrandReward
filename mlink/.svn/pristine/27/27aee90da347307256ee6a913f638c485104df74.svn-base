...
class Calculation extends LibFactory
{
    ...
    public function calTransaction()
    {
        ...
        //自定义属性
        !$this->isEmpty('advertiserStatus') && $params['store_type'] = $this->advertiserStatus;
        !$this->isEmpty('advertiserCooperationStatus') && $params['store_network_status'] = $this->advertiserCooperationStatus;
        !$this->isEmpty('advertiserLogoStatus') && $params['store_picture_status'] = $this->advertiserLogoStatus !=1 ? 'NO' : 'YES';
        !$this->isEmpty('advertiserPPCStatus') && $params['store_ppc_status'] = $this->advertiserPPCStatus;
        !$this->isEmpty('advertiserNameStatus') && $params['store_name_status'] = $this->advertiserNameStatus !=1 ? 'NO' : 'YES';
        !$this->isEmpty('publisherSiteType') && $params['publisher_type'] = $this->publisherSiteType;
        ...
        if (is_array($pubIds))
        {
            $params['pub_id'] = json_encode($pubIds);;
        }
        $advIds = $this->getAdvertiserIds();
        if (is_array($advIds))
        {
            $params['adv_id'] = json_encode($advIds);
        }
