<?php
    ...
    $REPORTWS_URI = 'https://www.tagadmin.sg/ws/AffiliateSOAP.wsdl';
    $CURRENCY = 'USD';
    ...
    $ua = new SoapVar(trim(USER_NAME), XSD_STRING, '', 'xsd:string');
    $pa = new SoapVar(trim(USER_PASS), XSD_STRING, '', 'xsd:string');
    ...
    if(count($result->Transactions->Transaction) > 1){
        foreach ($result->Transactions->Transaction as $val) {
            ...
            $dump[$reg_date][] = $TransactionDateTime . "\t" . $TransactionDateTime . "\t" . $val->OrderAmount . "\t" . $val->AffiliateCommissionAmount . "\t" . $val->MerchantId . "\t" . $val->MerchantName . "\t" . $val->AffiliateSubId . "\t" . $val->TransactionId . "\t" . $ClickDateTime . "\t" . $val->TransactionId . "\t" . $val->ApprovalStatus. "\t" . $val->TransactionType. "\t" . $val->OriginURL;
        }
    }else{
