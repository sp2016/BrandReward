...
function clear_invalid_data($d,$type,$lastversion){
    global $_db;
    if($type == 'VisitedDate'){
        $sql = "UPDATE statis_link SET c_orders = 0,c_sales = 0.0000 ,c_revenues = 0.0000,c_showrevenues = 0.0000,c_lastversion=0 WHERE createddate IN ('".join("','",$d)."') AND c_lastversion != '".$lastversion."'";
    }else{
        $sql = "UPDATE statis_link SET orders = 0,sales = 0.0000 ,revenues = 0.0000,showrevenues = 0.0000,lastversion=0 WHERE createddate IN ('".join("','",$d)."') AND lastversion != '".$lastversion."'";
    }
...
function update_visite_data($d,$lastversion){
    global $_db;
    $tmp_file = '/tmp/mysql/statis_finace_l.sql';
    if(file_exists($tmp_file)){
        unlink($tmp_file);
    }
    $sql = " SELECT VisitedDate as createddate,site,linkid,country,count(*) as orders,sum(Sales) as sales,sum(Commission) as revenues,sum(ShowCommission) as showrevenues  FROM rpt_transaction_unique WHERE VisitedDate IN ('".join("','",$d)."') AND af NOT IN ('bdg','mega','mk') GROUP BY VisitedDate,site,linkid,country into outfile '".$tmp_file."' fields terminated by '|' enclosed by '\"' lines terminated by '\r\n' ";
    // echo $sql."\n";
    $_db->query($sql);
    $fp = fopen($tmp_file,'r');
    ...
    while(!feof($fp)){
        ...
        $rows[] = array(
                ...
                'linkid'=>trim($data[2],'"'),
                'country'=>trim($data[3],'"'),
                'c_orders'=>trim($data[4],'"'),
                'c_sales'=>trim($data[5],'"'),
                'c_revenues'=>trim($data[6],'"'),
                'c_showrevenues'=>trim($data[7],'"'),
                'c_lastversion'=>$lastversion,
        );

        if(count($rows) > 99 ){
            $sql = getBatchUpdateSql($rows,'statis_link','createddate,site,linkid,country');
            $_db->query($sql);
            $rows = array();
        }
    }
    ...
    echo 'update statis_link in row : '.$count."\n";
}
...
function update_create_data($d,$lastversion){
    ...
    $sql = " SELECT CreatedDate as createddate,site,linkid,country,count(*) as orders,sum(Sales) as sales,sum(Commission) as revenues,sum(ShowCommission) as showrevenues  FROM rpt_transaction_unique WHERE CreatedDate IN ('".join("','",$d)."') AND af NOT IN ('bdg','mega','mk') GROUP BY CreatedDate,site,linkid,country into outfile '".$tmp_file."' fields terminated by '|' enclosed by '\"' lines terminated by '\r\n' ";
    // echo $sql."\n";
    $_db->query($sql);
    $fp = fopen($tmp_file,'r');
    ...
    while(!feof($fp)){
        ...
        $rows[] = array(
                ...
                'orders'=>trim($data[4],'"'),
                'sales'=>trim($data[5],'"'),
                'revenues'=>trim($data[6],'"'),
                'showrevenues'=>trim($data[7],'"'),
