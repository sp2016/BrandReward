...
function download_payments(){
    ...
    $url = 'https://www.tagadmin.sg/affiliate_invoice.html';
    _curl($url);
...
function _download_invoice($payments){
    global $file_temp;
    $url = "https://www.tagadmin.sg/affiliate_invoice_details_export.html?iId=".$payments['IDinNetwork']."&exportType=csv";
    _curl($url);
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if($line == 1){
                    ...
                    if(
                        ...
                        && strstr($data[6], "Order Value (SGD)")
                        && strstr($data[7], "Commission (SGD)")
...
function login(){
    $LoginUrl = 'https://www.tagadmin.sg/login.html';
