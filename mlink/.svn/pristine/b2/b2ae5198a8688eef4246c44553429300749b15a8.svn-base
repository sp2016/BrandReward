...
function option_move(fboxid,tboxid,sortitems)
{
	...
	for(var i=0;i<fbox.options.length;i++)
	{
		...
		{
//    		if(checkOptionExist(tboxid, fbox.options[i].value)){
//				continue;
//			}
...
function checkOptionExist(objid, val){
	objid = document.getElementById(objid);
	for(var i=0; i<objid.options.length; i++)
	{
		if(objid.options[i].value == val){
			return true;
		}
	}
	return false;
...
function parseAjaxResult(msg)
{
	var result = {"status":0,"html":""};
	if(typeof msg != "string") result;
	if(msg.match(/^status:(.*)/i))
	{
		result.status = RegExp.$1;
		result.html = msg.replace(/^status:(.*)\n/,"");
	}
	else
	{
		result.html = msg;
