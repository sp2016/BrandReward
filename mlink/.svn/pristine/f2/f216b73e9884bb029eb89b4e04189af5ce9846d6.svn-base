...
try {
	...
	while (strtotime($beginDate) <= strtotime($end_dt))
	{
		$endDate = date('Y-m-d', strtotime('+1 days', strtotime($beginDate)));;
		$url = "https://api.awin.com/publishers/".PUBLISH_ID."/transactions/?startDate=".$beginDate."T00%3A00%3A00&endDate=".$endDate."T00%3A00%3A00&timezone=UTC&accessToken=".API_TOKEN;
