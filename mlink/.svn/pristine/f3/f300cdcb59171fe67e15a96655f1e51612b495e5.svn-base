...
if(isset($_POST['id']) && isset($_POST['domain']))
{
	$id = $_POST['id'];
	$domain = $_POST['domain'];
	$sql = "SELECT ExtUrl FROM publisher_domain_detail WHERE DomainInfoID='$id' AND ExtDomain = '$domain'";
...
if($type == 1){
    $title = 'Publisher Site Analysis';
    $list = $domain->urldata($_GET,$p,$pagesize);
    $seachtype = 1;
    if($list == 'No Data'){
        $type1 = 'null';
        $list1 = 'No Data';
    ...
    $list = $domain->urldetaildata($_GET,$p,$pagesize);
    $seachtype = 2;
    if($list == 'No Data'){
        $type = 'null';
        $list1 = 'No Data';
...
$objTpl->display('b_publisher_site.html');