...
$tran_data = $objTran->get_aff_ov($_GET);
...
if(!isset($_GET['sel_site']) || empty($_GET['sel_site']) || in_array('All',$_GET['sel_site'])){
}else{
	foreach($_GET['sel_site'] as $k=>$v){
		$param[] = urlencode('sel_site[]').'='.urlencode($v);
	}
}
$param_str = join('&',$param);
$daily_url = BASE_URL.'/aff_daily.php?'.$param_str;
