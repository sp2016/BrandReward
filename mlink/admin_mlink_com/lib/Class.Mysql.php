...
class Mysql
{
	...
	function query($sql) {
		...
		$result['sql'] = $sql;
		$result['start'] = microtime(true);      //返回当前 Unix 时间戳的微秒数
		$this->queryID = @mysql_query($sql, $this->linkID);//@是错误控制符，加了@，就不会报错
		$result['end'] = microtime(true);
		$result['time'] = number_format($result["end"]-$result["start"], 3, '.', ' ');
		$result['parent'] = __FUNCTION__;              //返回函数名
	...
	function getRows(&$sql, $keyname = '', $foundrows = false) {
		$arr_return = array();
		if ($foundrows && strpos(substr($sql,0,30), 'SQL_CALC_FOUND_ROWS') === false) {//SQL_CALC_FOUND_ROWS表示符合要求的所有记录
		...
		while ($row = mysql_fetch_array($qryId, MYSQL_ASSOC)) {// $qryId是结果集，MYSQL_ASSOC代表返回的是关联数组
			if ($keyname) {
				$arr_temp = array();
				foreach ($keys as $key)
					$arr_temp[] = $row[$key];
