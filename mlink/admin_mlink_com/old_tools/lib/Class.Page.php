...
//	var $default_onepage = 100;
	var $default_onepage = 50;
    function opb($total = 0, $onepage = "", $form_vars = '', $max_per_page = 500) 
    {
    	if(isset($_GET["onepage"])){
    		if(intval($_GET["onepage"]) < 1){
    			$onepage = 1;
    		}else{
    			$onepage = intval($_GET["onepage"]);
    		}
    		if (empty($onepage) || $onepage > $max_per_page) $onepage = $max_per_page;
    		setcookie("onepage", $onepage, time()+60*60*24*30);
    	}else{
			if (!isset($_COOKIE['onepage']) || empty($_COOKIE['onepage'])) {
				if (empty($onepage)){$onepage = $this->default_onepage;}
				if ($onepage > $max_per_page) $onepage = $max_per_page;
				setcookie("onepage", $onepage, time()+60*60*24*30);
