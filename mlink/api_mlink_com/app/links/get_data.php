...
if(isset($_req['country']) && !empty($_req['country'])){
    $country_str = ' AND Country = "'.addslashes($_req['country']).'"';
}

$sql = 'SELECT COUNT(*) as c FROM c_content_feed WHERE IsActive="YES"'.$country_str;
$row = $_db->getRows($sql);
$all = $row[0]['c'];
...
$sql = 'SELECT ID,Title,Code,`Type`,Url,Advertiser_Name,ImgIsDownload,ImgFile as Images,StartTime,ExpireTime,TimeZone,CreateTime as Created,UpdateTime as Updated FROM c_content_feed WHERE IsActive="YES" '.$country_str.' LIMIT '.($res['page']-1)*$res['page_size'].','.$res['page_size'];
