...
$file = array();
$api_url = 'http://api.i.meikaiinfotech.com/?act=mlink.get_clicks&date=[datetime]';
$content = '';

$colums_in = "source,sourceid,alias,created,domainId,programId,programIdInAff,programName,sessionId,publishTracking,affId,createddate,site";
...
foreach($date_range as $date){
	...
	$http_data['file_temp'] = DATA_ROOT.'mkoutlog/clicks_'.str_replace('-','',$date).'.dat';
        _http($url,$http_data);
	$sql = "load data infile  '".$http_data['file_temp']."' REPLACE into table bd_out_tracking_inner fields terminated by '|' enclosed by '\"' lines terminated by '\r\n' (".$colums_in.")";
	$_db->query($sql);

        $sql = "UPDATE bd_out_tracking SET affId = 9999,programId = 14018 WHERE createddate = '".$date."' AND sessionid IN (SELECT DISTINCT(PublishTracking) FROM bd_out_tracking_inner WHERE createddate = '".$date."' AND affId >0 AND affId != 191) ";
        $_db->query($sql);

        $sql = "UPDATE bd_out_tracking_min SET affId = 9999,programId = 14018 WHERE createddate = '".$date."' AND sessionid IN (SELECT DISTINCT(PublishTracking) FROM bd_out_tracking_inner WHERE createddate = '".$date."' AND affId >0 AND affId != 191) ";
        $_db->query($sql);
}
