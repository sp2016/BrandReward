...
class MysqlExt
{
	...
	function getDescTable($_table_name)
	{
		//Field,Type,Null,Key,Default,Extra
		$sql = "DESC `$_table_name`";
		return $this->oMysql->getRows($sql,"Field");
	}
	
	function getCreateTableSql($_table_name)
	{
		$sql = "SHOW CREATE TABLE `$_table_name`";
		return $this->oMysql->getFirstRowColumn($sql,"Create Table");
	}
	...
	function isTableExisting($_table_name)
	{
		...
		if($this->oMysql->getFirstRowColumn($sql)) return true;
		return false;
	}
	...
	function getTableIndex($_table_name)
	{
		...
		$arrRow = $this->oMysql->getRows($sql);
