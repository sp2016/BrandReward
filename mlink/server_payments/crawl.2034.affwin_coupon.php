...
$currency_map = array(
	...
	'USD'=>array('page_id'=>3,'download_tmp'=>'/affiliate/'.ACCOUNT_ID.'/report/transactions/export/network/awin/dateRange/lifetime/transRegions/US/searchQueryField/payment_id/searchQuery/'),
...
function download_payments(){
    ...
    foreach($currency_map as $k=>$v){
        ...
        if(empty($mTable) || !isset($mTable[0])){
            echoMsg('crawl payments html fail','crawl payments','fail');
            continue;
        }

        preg_match_all('/<tr class=.*?<\/tr>/is',$mTable[0],$mTr);
        if(empty($mTr) || !isset($mTr[0])){
            echoMsg('crawl payments tr fail','crawl payments','fail');
            continue;
        }
        ...
        foreach($mTr[0] as $tr){
            ...
            if(empty($mTd) || !isset($mTd[0])){
                echoMsg('crawl payments td fail','crawl payments','fail');
                continue;
            }
            ...
            if(empty($mDOWNURL) || !isset($mDOWNURL[1])){
                echoMsg('crawl payments id fail','crawl payments','fail');
                continue;
            }
...
function login(){
    ...
    $loginHtml = file_get_contents($file_temp);
