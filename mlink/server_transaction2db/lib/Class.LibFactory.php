...
//             .$this->parseLock(isset($options['lock'])?$options['lock']:false);
    ...
    function getBatchUpdateSql($updateData,$tableName,$primary=''){
        ...
        $sql = '';
        if(!empty($updateData)){
            $column_arr = array_keys($updateData[0]);
            $column_str = join(',',$column_arr);
            $sql = 'INSERT INTO '.$tableName.' ('.$column_str.') VALUES ';
            foreach($updateData as $k=>$row){
                $values_arr = array();
                foreach($column_arr as $col){
                    if(!isset($row[$col])){
                        return 'error: wrong update data';
                    }
                    $values_arr[] = addslashes($row[$col]); 
                }
    ...
    function getInsertSql($updateData,$tableName,$replace=false){
        ...
        if(!empty($updateData)){
            ...
            $sql .= ' INTO '.$tableName.' ('.$columnStr.') VALUES ';
            foreach($updateData as $k=>$row){
