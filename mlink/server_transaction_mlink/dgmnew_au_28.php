...
define('USER_NAME', "IRRpkMxEbHdJ345716EhMkqsdHwm5coaR3");
define('USER_PASS', "+yoHa(Ppqrj2otadLPJJoi7KjKxPEBjg");
...
$change = strtotime($start_date.'T00:00:00+10:00');
$start_dt = date("Y-m-d",$change)."T".date("H:i:s",$change)."-08:00";
$change = strtotime($end_date.'T23:59:59+10:00');
$end_dt = date("Y-m-d",$change)."T".date("H:i:s",$change)."-08:00";
...
do {
    ...
    foreach ($json['Actions'] as $v) {
        ...
        preg_match('/(\d{4}-\d{2}-\d{2})/i',trim($v['EventDate']),$matches);
        $day = $matches[1];
        //echo $v['Currency'].PHP_EOL;
        ...
        $replace_array = array(
            '{createtime}'      => $day.' 00:00:00', //date('Y-m-d H:i:s', strtotime(trim($v['EventDate']))),
            '{updatetime}'      => $day.' 00:00:00',//date('Y-m-d H:i:s', strtotime(trim($v['CreationDate']))),
            '{sales}'           => $sales,
            '{commission}'      => $rev,
            ...
            '{sid}'             => trim($v['SubId1']),
            '{orderid}'         => '',
            '{clicktime}'       => $day.' 00:00:00',//date('Y-m-d H:i:s', strtotime(trim($v['EventDate']))),
            '{tradeid}'         => trim($v['Id']),
