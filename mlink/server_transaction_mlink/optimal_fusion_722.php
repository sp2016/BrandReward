...
define('AffID', '6083');
define('API_Key', 'T6FgkaNv2HwC3zScvchjrg');
define('API_URL','http://login.optimalfusion.com/affiliates/api/5/reports.asmx/Conversions?');
...
if (empty($xml['conversions'])) {
	mydie("Transaction is empty!, please check it.");
}
if ($xml['row_count'] != count($xml['conversions']['conversion']))
	mydie("returned counts error of data");
	
//get offer_contract_id
$offer_url = "http://login.optimalfusion.com/affiliates/api/4/offers.asmx/OfferFeed?api_key=".API_Key."&affiliate_id=".AffID."&campaign_name=&media_type_category_id=0&vertical_category_id=0&vertical_id=0&offer_status_id=0&tag_id=0&start_at_row=1&row_limit=0";
...
foreach ($xml['conversions']['conversion'] as $v){
	...
	$Commission = round($v['price'], 2);
	$tradestatus = (!empty($v['disposition']))?$v['disposition']:'';
