<?php
    ...
    $REPORTWS_URI = 'https://www.tagpm.eu/ws/AffiliateSOAP.wsdl';
    ...
    if(count($result->Transactions->Transaction) > 1){
        foreach ($result->Transactions->Transaction as $val) {
            ...
            $dump[$reg_date][] = $SaleApprovalDateTime . "\t" . $TransactionDateTime . "\t" . $val->OrderAmount . "\t" . $val->AffiliateCommissionAmount . "\t" . $val->MerchantId . "\t" . $val->MerchantName . "\t" . $val->AffiliateSubId . "\t" . $val->TransactionId . "\t" . $ClickDateTime . "\t" . $val->TransactionId . "\t" . $val->ApprovalStatus. "\t" . $val->TransactionType;
        }
    }else{
