...
function getDictionary($type){
	...
	if(isset($dictionaryData[$type])){
	    ...
	    if($type == 'country'){
	        ...
	        $tmp = $objAccount->table('country_codes')->find();
	        if($tmp){
	            foreach($tmp as $v){
	                $data[$v['id']] = $v['CountryName'];
	            }
...
function user_access_record(){
    $publisherId = isset($_SESSION['u']['ID'])?$_SESSION['u']['ID']:0;
    $php_url = basename($_SERVER['PHP_SELF']);
    $host = $_SERVER["HTTP_HOST"];
    $ip = $_SERVER["REMOTE_ADDR"];
    $sql = "insert into publisher_access_record(`PublisherId`,`Host`,`Address`,`IP`,`AddTime`) VALUES('".$publisherId."','".$host."','".$php_url."','".$ip."','".date("Y-m-d H:i:s")."')";
    mysql_query($sql);
}
