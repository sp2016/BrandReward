...
class LinkFeed_124_TAG_UK
{
	var $info = array(
		"ID" => "124",
		"Name" => "TAG UK",
		"IsActive" => "YES",
		"ClassName" => "LinkFeed_124_TAG_UK",
	...
	function getCouponFeed()
	{
		...
		while ($HasNextPage)
		{
			$url = "https://www.tagpm.com/affiliate_program_vouchers_grid.html?responseType=json&pageSize=$PageSize&p=$Page";
			...
			foreach ($r['results'] as $v)
			{
				...
				$link = array(
						...
						"LinkStartDate" =>  isset($v['validityStartDate']) ? parse_time_str($v['validityStartDate'],'d/m/Y') : '',
						"LinkEndDate" => isset($v['validityEndDate']) ? parse_time_str($v['validityEndDate'],'d/m/Y') : '',
	...
	function GetAllLinksByAffId(){
	    ...
	    foreach ($arr_merchant as $merinfo){
	        ...
	        $url = sprintf('https://www.tagpm.com/affiliate_program_creative.html?pId=%s', $merinfo['IdInAff']);
	        ...
	        foreach ($chapters[1] as $chapter)
	        {
	            ...
	            $link['LinkAffUrl'] = sprintf('https://www.tagserve.com/clickServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s&SUBID=' ,
	                $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	            switch ($v[2])
	            {
	                case 'Banner';
	                $link['LinkImageUrl'] = sprintf('https://www.tagserve.com/impressionServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s',
	                    $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	                break;
	                case 'Text Link':
	                    break;
	...
	function GetProgramByPage()
	{
		...
		$r = $this->oLinkFeed->GetHttpResult("https://www.tagpm.com/affiliate_directory.html",$request);
		$result = $r["content"];
		...
		while ($nLineStart >= 0){
			...
			$prgm_url = "https://www.tagpm.com/affiliate_program_detail.html?pId=$strMerID";
