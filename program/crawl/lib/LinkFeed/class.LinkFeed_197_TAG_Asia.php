...
class LinkFeed_197_TAG_Asia
{
	...
	function getCouponFeed()
	{
		...
		while ($HasNextPage)
		{
			$url = "https://www.tagadmin.asia/affiliate_program_vouchers_grid.html?responseType=json&pageSize=$PageSize&p=$Page";
	...
	function GetAllLinksByAffId(){
	    ...
	    foreach ($arr_merchant as $merinfo){
	        ...
	        $url = sprintf('https://www.tagadmin.asia/affiliate_program_creative.html?pId=%s', $merinfo['IdInAff']);
	        ...
	        foreach ($chapters[1] as $chapter)
	        {
	            ...
	            $link['LinkAffUrl'] = sprintf('https://www.tagserve.asia/clickServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s&SUBID=' ,
	                $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	            switch ($v[2])
	            {
	                case 'Banner';
	                $link['LinkImageUrl'] = sprintf('https://www.tagserve.asia/impressionServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s',
	                    $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	                break;
	                case 'Text Link':
	                    break;
