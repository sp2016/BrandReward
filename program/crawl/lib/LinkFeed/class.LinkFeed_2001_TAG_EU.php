...
class LinkFeed_2001_TAG_EU
{
	var $info = array(
		"ID" => "2001",
		"Name" => "TAG EU",
		"IsActive" => "YES",
		"ClassName" => "LinkFeed_2001_TAG_EU",
	...
	function getCouponFeed()
	{
		...
		while ($HasNextPage)
		{
			$url = "https://www.tagpm.eu/affiliate_program_vouchers_grid.html?responseType=json&pageSize=$PageSize&p=$Page";
	...
	function GetAllLinksByAffId(){
	    ...
	    foreach ($arr_merchant as $merinfo){
	        ...
	        $url = sprintf('https://www.tagpm.eu/affiliate_program_creative.html?pId=%s', $merinfo['IdInAff']);
	        ...
	        foreach ($chapters[1] as $chapter)
	        {
	            ...
	            $link['LinkAffUrl'] = sprintf('https://www.tagserve.eu/clickServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s&SUBID=' ,
	                $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	            switch ($v[2])
	            {
	                case 'Banner';
	                $link['LinkImageUrl'] = sprintf('https://www.tagserve.eu/impressionServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s',
	                    $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	                break;
	                case 'Text Link':
	                    break;
	...
	function GetProgramByPage()
	{
		...
		$r = $this->oLinkFeed->GetHttpResult("https://www.tagpm.eu/affiliate_directory.html",$request);
		$result = $r["content"];
		...
		while ($nLineStart >= 0){
            ...
            if(!$this->getStatus) {
                ...
                $prgm_url = "https://www.tagpm.eu/affiliate_program_detail.html?pId=$strMerID";
