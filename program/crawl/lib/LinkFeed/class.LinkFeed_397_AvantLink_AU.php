...
class LinkFeed_397_AvantLink_AU
{
	function __construct($aff_id,$oLinkFeed)
	{
		...
		if(SID == 'bdg01'){
			$this->AffiliateID = '166475';
			$this->WebiteID = '208111';
			$this->SubscriptionID = '68289';
			$this->API_key = '764e3bb6a02ddb5bc68eecdd59835be7';
		}else{
			$this->AffiliateID = '172813';
			$this->WebiteID = '220817';
			$this->SubscriptionID = '68961';
			$this->API_key = '8cecfc696808d56a45ae20eb35e0ac23';
		}
	}	
	...
	function Login(){
		...
		$r = $this->oLinkFeed->GetHttpResult($this->info["AffLoginUrl"], $request);		//print_r($r);exit;
		if($r["code"] == 200){
	...
	function getMerchantByStatus($status,&$arrAllExistsMerchants)
	{
		...
		$strUrl = "https://www.avantlink.com.au/affiliate/merchants.php";
	...
	/* function getCouponFeed()
	{
		...
		if(!$this->oLinkFeed->fileCacheIsCached($cache_file))
		{
			...
			$strUrl = "http://www.avantlink.com.au/coupons/coupon_feed.php?cfi=4257&pw=4724";
			$request["method"] = "get";
			$r = $this->oLinkFeed->GetHttpResult($strUrl,$request);
			$result = $r["content"];
	...
	function getProgramByStatus($status, $country)
	{
		...
		/* if($country == "ca"){
			...
			$domain = "https://classic.avantlink.com.au";
			$TargetCountryExt = 'AU';
		}else{
	...
	function GetProgramByPage()
	{
		...
		foreach($arrStatus4List as $status)
		{
			$program_num += $this->getProgramByStatus($status, "au");
		}

		echo "\tGet Program by page end\r\n";

		if($program_num < 3){
			mydie("die: program count {$program_num} < 3, please check program.\n");
		}
