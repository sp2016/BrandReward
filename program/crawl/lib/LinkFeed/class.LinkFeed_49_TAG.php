...
class LinkFeed_49_TAG
{
    var $info = array(
        "ID" => "49",
        "Name" => "TAG",
        "IsActive" => "YES",
        "ClassName" => "LinkFeed_49_TAG",
	...
	function getCouponFeed()
	{
		...
		while ($HasNextPage)
		{
			$url = "https://www.tagadmin.com.au/affiliate_program_vouchers_grid.html?responseType=json&pageSize=$PageSize&p=$Page";
	...
	function GetAllLinksByAffId(){
	    ...
	    foreach ($arr_merchant as $merinfo){
	        ...
	        $url = sprintf('https://www.tagadmin.com.au/affiliate_program_creative.html?pId=%s', $merinfo['IdInAff']);
	        ...
	        foreach ($chapters[1] as $chapter)
	        {
	            ...
	            $link['LinkAffUrl'] = sprintf('https://www.tagserve.com.au/clickServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s&SUBID=',
	                $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	            switch ($v[2]) {
	                case 'Banner';
	                $link['LinkImageUrl'] = sprintf('https://www.tagserve.com.au/impressionServlet?AID=%s&MID=%s&PID=%s&SID=%s&CID=%s',
	                    $aid, $mid, $merinfo['IdInAff'], $this->sid, $link['AffLinkId']);
	                break;
	                case 'Text Link':
	                    break;
    ...
    function GetProgramByPage()
    {
        ...
        /*$request = array(
            "AffId" => $this->info["AffId"],
            "method" => "post",
            "postdata" => "username=info%40couponsnapshot.co.uk&password=6yU12RhoujmBU&submit=Login",
        );

        $r = $this->oLinkFeed->GetHttpResult("https://www.tagpm.com/login.html",$request);*/
        ...
        $r = $this->oLinkFeed->GetHttpResult("https://www.tagadmin.com.au/affiliate_directory.html", $request);
        $result = $r["content"];
        ...
        while ($nLineStart >= 0) {
            ...
            if(!$this->getStatus) {
                ...
                $prgm_url = "https://www.tagadmin.com.au/affiliate_program_detail.html?pId=$strMerID";
                ...
                if(preg_match('/Coupon codes may be used as long as they are approved and distributed to you from the Advertiser and account manager directly./',$TermAndCondition)){
                    $AllowNonaffCoupon ='NO';
                }else if(preg_match('/Affiliates must only use the marketing copy approved by the merchant, approved copy is provided in the affiliate resource section./',$TermAndCondition)){
                    $AllowNonaffCoupon ='NO';
                    $AllowNonaffPromo ='NO';
                }
                //通用条件
