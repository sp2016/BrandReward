...
class LinkFeed_722_Optimal_Fusion
{
	function __construct($aff_id, $oLinkFeed)
	{
		...
		$this->DataSource = 434;
		if(SID == 'bdg02'){
			$this->AffiliateID = 6083;
			$this->API_Key = 'T6FgkaNv2HwC3zScvchjrg';
		}else{
			$this->AffiliateID = 6031;
			$this->API_Key = 'cGcPFZCb5Z70p5y1pqNEKQ';
		}
	}
	...
	function GetAllLinksByAffId()
	{
		...
		$offerDeed_url = "http://login.optimalfusion.com/affiliates/api/4/offers.asmx/OfferFeed?api_key={$this->API_Key}&affiliate_id={$this->AffiliateID}&campaign_name=&media_type_category_id=0&vertical_category_id=0&vertical_id=0&offer_status_id=0&tag_id=0&start_at_row=1&row_limit=0";
		...
		foreach ($xml->offers->offer as $v)
		{
			...
			if(empty($campaign_id))
				continue;
			$link_url = "https://login.optimalfusion.com/affiliates/api/2/offers.asmx/GetCampaign?api_key={$this->API_Key}&affiliate_id={$this->AffiliateID}&campaign_id={$campaign_id}";
			...
			foreach ($linkxml->campaign->creatives->creative_info as $row)
			{
				...
				if($row->creative_type->type_name == 'Image'){
					...
					$detailUrl = "http://login.optimalfusion.com/affiliates/Extjs.ashx?s=getcode&cid={$row->creative_id}&conid={$campaign_id}&_dc={$timestrmp}";
	...
	function GetProgramByApi()
	{
		...
		foreach ($xml->offers->offer as $v)
		{
			...
			//print_r($arr_prgm[$strMerID]['TargetCountryExt']."\r\n");
			$program_num++;
