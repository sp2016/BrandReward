...
class LinkFeedDb
{
	...
	function getAffById($aff_id)
	{
        ...
        $sql = "select Account,Password,Alias,TransactionApiKey,TransactionDefaultCurrency from wf_aff where ID = ".$aff_id;
        ...
        $arr['Alias'] = $aff_AP['Alias'];
        $arr['TransactionApiKey'] = $aff_AP['TransactionApiKey'];
        $arr['TransactionDefaultCurrency'] = $aff_AP['TransactionDefaultCurrency'];
...
//		$this->objMysql->query($sql);
	...
	function getProductTableName($aff_id){
	    if(!is_numeric($aff_id)) mydie("die:getProductTableName($aff_id) failed\n");
	    if(isset($this->product_tables[$aff_id])) return $this->product_tables[$aff_id];
	    
	    $product_table_name = "affiliate_product_" . $aff_id;
	    if(! $this->objMysql->isTableExisting($product_table_name))
	    {
	        $this->objMysql->duplicateTable("affiliate_product_default",$product_table_name);
	    }
	    $this->link_tables[$aff_id] = $product_table_name;
	...
	function UpdateProductToDB(&$arr_list){
		
		if(!is_array($arr_list) || !isset($arr_list[0]))
			mydie("UpdateProductToDB failed: array is needed. <br>\n");
		$first_row = $arr_list[0];
		if(!isset($first_row["AffId"]))
			mydie("UpdateProductToDB failed: AffId not provided. <br>\n");
		if(!isset($first_row["ProductName"]) || $first_row["ProductName"] == "")
			mydie("UpdateProductToDB failed: ProductName not provided. <br>\n");
		if(!isset($first_row["AffProductId"]) || $first_row["AffProductId"] == "")
			mydie("UpdateProductToDB failed: AffProductId not provided. <br>\n");
		$aff_id = $first_row["AffId"];
		
	    $arr_col = array("AffId","AffMerchantId", "AffProductId","ProductName","ProductCurrency","ProductPrice","ProductOriginalPrice","ProductRetailPrice","ProductImage","ProductLocalImage","ProductUrl","ProductDestUrl","ProductDesc","CommissionExt","Language","ProductStartDate","ProductEndDate","LastUpdateTime","AddTime");
	    $col_for_check = array("ProductName", "ProductCurrency", "ProductPrice","ProductLocalImage","ProductUrl","ProductDestUrl","ProductDesc","CommissionExt","Language","ProductStartDate","ProductEndDate");
	    $product_table_name = $this->getProductTableName($aff_id);//生成productfeed表名
	    
	    $sql_check_pre = "SELECT " . implode(",",$col_for_check) . " FROM $product_table_name WHERE ";
	    $arr_insert = array();
	    $count = 0;
	    $updated = 0;
	    foreach($arr_list as $row){
	        ...
	        //check
	        $sql_check = $sql_check_pre . "AffId = ".$row["AffId"]."  AND AffMerchantId = '".addslashes($row["AffMerchantId"])."' AND AffProductId = '".addslashes($row["AffProductId"])."'";
	        $result = $this->objMysql->getFirstRow($sql_check);
	        if(isset($result["ProductName"])){//说明数据表中已经有这个product，更新与否，首先看ProductName，其次看各个字段
	            $arr_changed = array();
	            $arr_changed_col = array();
	            foreach($col_for_check as $col){//检查那个字段有改变
	                $is_same = false;
	                if($col == "ProductStartDate" || $col == "ProductEndDate"){
	            ...
	            if(sizeof($arr_changed)){
	                ...
	                $arr_changed_col['isactive'] = "isactive = 'YES'";
	                $arr_changed_col['LastChangeTime'] = "LastChangeTime = '" . date('Y-m-d H:i:s') . "'";//edit time 2017/03/28
	                $sql = "update $product_table_name set " . implode(",",$arr_changed_col) . ",LastUpdateTime = '".date('Y-m-d H:i:s')."' WHERE AffId = ".$row["AffId"]." AND AffMerchantId = '".addslashes($row["AffMerchantId"])."' AND AffProductId = '".addslashes($row["AffProductId"])."'";
	                $this->objMysql->query($sql);//将有改变的字段进行更新
	            }else{
	                $sql = "update $product_table_name set LastUpdateTime = '".date('Y-m-d H:i:s')."', isactive = 'YES'  WHERE AffId = ".$row["AffId"]." AND AffMerchantId = '".addslashes($row["AffMerchantId"])."' AND AffProductId = '".addslashes($row["AffProductId"])."'";
	                $this->objMysql->query($sql);
	            }
	        }else{
	            //for insert
	    ...
	    if(sizeof($arr_insert)){
	        $sql = "insert ignore into $product_table_name(" . implode(",",$arr_col) . ") values " . implode(",",$arr_insert);
	        $this->objMysql->query($sql);
	    }
	    echo sprintf("updating Product(%s)... New:%s, Updated:%s, Unchanged:%s.\n", count($arr_list), count($arr_insert), $updated, count($arr_list)-count($arr_insert)-$updated);
	    return $count;
		
	}
	
	function checkProductExists($aff_id, $check_date)
	{
	    //FOR mlink
	    $where = " AND `AffId` = '".$aff_id."'";;
	     
	    if(SID != 'bdg01'){
	        $product_table_name = $this->getProductTableName($aff_id);
	        
	        //减掉的
	        $sql = "select count(*) from `$product_table_name` where LastUpdateTime < '$check_date' and isactive = 'YES' $where";
	        ...
	        if($activeCount==0 || ($activeCount && floor(($cnt/$activeCount)*100)>$threshold)){
	            ...
	            AlertEmail::SendAlert( $aff_id.': Crawl Product',nl2br("total count:$activeCount,inactive count:$cnt"), $to);
	            //mydie("die: too many product Inactive: $cnt .\n");
	            //exit;
	        }//else{
	            $sql = "update `$product_table_name` set isactive = 'NO', LastChangeTime = '".date('Y-m-d H:i:s')."' where  LastUpdateTime < '$check_date' and isactive = 'YES' $where";
	            $this->objMysql->query($sql);
	            echo "\tSet  $cnt Products Inactive.\r\n";
	        //}
	    }
	}
	...
	function UpdateLinkToDB(&$arr_list)
	{
		...
		$col_for_check = array("LinkName", "LinkDesc", "LinkStartDate", "LinkEndDate", "LinkPromoType", "LinkOriginalUrl","LinkImageUrl","LinkAffUrl","LinkCode", "Type", "IsDeepLink","IsActive");
		...
		foreach($arr_list as $row){
			...
			if(isset($result["LinkName"])){//说明数据表中已经有这个link，更新与否，首先看linkName，其次看各个字段
				...
				if(sizeof($arr_changed)){
					...
					//FOR mlink
					if(SID != 'bdg01'){
					    if($result['IsActive'] == 'NO'){ //lastchangetime 更新
					        $sql = "update $link_table_name set LastUpdateTime = '".date('Y-m-d H:i:s')."', isactive = 'YES', LastChangeTime = '".date('Y-m-d H:i:s')."',LinkName = '".addslashes($row['LinkName'])."', LinkDesc = '".addslashes($row['LinkDesc'])."'  where AffMerchantId = '".addslashes($row["AffMerchantId"])."' AND AffLinkId = '".addslashes($row["AffLinkId"])."'";
					    }
						else{
	...
	function checkLinkExists($aff_id, $check_date, $type='', $AffMerchantId=0)
	{
		...
		if(SID != 'bdg01'){
			...
			if($aff_id == 20 || $aff_id == 152 || $aff_id == 415 || $aff_id == 2028 ){
			    $threshold = 20;
			}
			else{
			    $threshold = 10;
			...
			if($activeCount==0 || ($activeCount && floor(($cnt/$activeCount)*100)>$threshold)){
			    $sql = "INSERT INTO crawl_links_logs (Affid, `Type`, Amount, ToInactive, CheckTime) 
			            VALUES ({$aff_id}, '".addslashes($type)."', $activeCount, '".$cnt."', '".$check_date."')";
			    $this->taskDB->query($sql);
			    echo ("die:Too many links will be cut, Crawl count:$activeCount, Will be cut count:$cnt .\n");
			}else{
...
//				echo "\n";
	...
	function product_program_num_config($affid=''){
	     
	    $sql = "select * from aff_product_count_config WHERE `Status`='YES'";
	    $storeConfig = $this->objMysql->getRows($sql);
	    $Arr = array();
	    foreach ($storeConfig as $value){
	         
	        $sql = "SELECT b.id,b.name,b.affid,b.idinaff FROM r_store_program a left join program b on a.programid = b.id where a.StoreId = {$value['StoreId']}";
	        $tmpArr = $this->taskDB->getRows($sql);
	        foreach ($tmpArr as $tV){
	            if(!$tV['id']) continue;
	            $Arr[$tV['affid']][$tV['idinaff']] = array(
	                'affid' => $tV['affid'],
	                'idinaff' => $tV['idinaff'],
	                'limit' => $value['Limit'],
	            );
	    ...
	    if($affid){
	        return isset($Arr[$affid]) ? $Arr[$affid]:array();
	    }
	    return $Arr;
	}
    ...
    function cur_exchange($fcu, $tcu, $date)
    {
        ...
        if (!isset($CURRENCY_HIS[$k][$date])) {
            ...
            $result = $this->taskDB->getFirstRow($sql);
            $CURRENCY_HIS[$k][$date][$result['NAME']] = $result['EXCHANGERATE'];
