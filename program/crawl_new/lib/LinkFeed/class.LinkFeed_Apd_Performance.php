 ...
 * Time: 17:13
 */
class LinkFeed_Apd_Performance
{
    ...
    function GetProgramByApi($SiteID, $SiteIdInAff, $APIKey)
    {
        ...
        while ($hasNextPage) {
            ...
            $strUrl = sprintf('https://%s:%s@api.impactradius.com/2010-09-01/Mediapartners/%s/Campaigns.json?PageSize=%s&Page=%s', urlencode($SiteIdInAff), urlencode($APIKey), urlencode($SiteIdInAff), $perPage, $page);
            ...
            foreach ($data['Campaigns'] as $val) {
                ...
                $merDetailUrl = sprintf("https://www.apdperformance.com.au/secure/mediapartner/campaigns/campaign.ihtml?d=lightbox&c=%s&fromPopup=1", $ProgramID);
                ...
                $ContactEmail = 'https://www.apdperformance.com.au' . trim($this->oLinkFeed->ParseStringBy2Tag($result, 'parent.uitkPopup("', '"'));
                ...
                $termsUrl = sprintf('https://www.apdperformance.com.au/secure/mediapartner/campaigns/mp-view-io-by-campaign-flow.ihtml?c=%s',$ProgramID);
