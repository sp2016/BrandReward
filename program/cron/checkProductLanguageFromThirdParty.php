...
$sql = "SELECT id, ProductName as title, ProductDesc as `desc` FROM product_feed WHERE `status` = 'active' and `language` = 'en' AND ischecklanguage = 'NO'";
...
foreach ($data as $value) {
    ...
    $updateSql = "update product_feed set IsCheckLanguage = 'YES' where id = {$value['id']}";
    ...
    if($firstLanguage == $secondLanguage){
        ...
        if(in_array($language, $languageArr)){
            $updateSql = "update product_feed set `language` = '{$language}', IsFixLanguage = 'YES' where id = {$value['id']}";
            $objProgram->objMysql->query($updateSql);
