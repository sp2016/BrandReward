...
//	}
	...
	while(!$check_ps)
	{
		...
		if(!strlen($pid_list) || !strlen($logid_list)){
			...
			$log_p = array();
			//TODO 判断条件：r_domain_program LastUpdateTime
			...
			//TODO 判断条件：program_manual LastUpdateTime
			$log_p = array();
			...
			//TODO 判断条件：program LastUpdateTime
			$log_p = array();
			...
			//$sql = "select a.id as pid from program a where a.affid = $affid and (a.AddTime >= '".date("Y-m-d H:i:s", strtotime(" -5 minutes"))."')";
			$log_p = $objProgram->objMysql->getRows($sql, "pid");
			if(count($log_p))
				$pid_list = array_merge($pid_list ,array_keys($log_p));
			
			//TODO 判断条件：in program but not in program_intell
...
//				$sql = "select a.`ID` pid from program a left join program_intell b on a.`ID`=b.`ProgramId` where a.`AffId` = $affid and b.`ProgramId` is null";
			...
			//TODO 疑问，a.isactive = 'active'不需要
			//isactive not correct
			$log_p = array();
			$sql = "SELECT a.programid FROM program_intell a INNER JOIN program b ON a.programid = b.id WHERE a.affid = '$affid' AND a.isactive = 'active' AND (b.statusinaff <> 'active' AND b.partnership <> 'active') AND a.programid NOT IN (SELECT programid FROM program_manual WHERE statusinbdg = 'active')";
			$log_p = $objProgram->objMysql->getRows($sql, "programid");
			if(count($log_p))
				$pid_list = array_merge($pid_list ,array_keys($log_p));
			
			//TODO 判断条件：program_intell inactive but program is active and program_manual is not inactive
			$log_p = array();
			...
			//TODO 判断条件：program is active but program_intell is inactive
		...
		$prgm_arr = array();
		$prgm_arr = $objProgram->getProgramInfoById($pid_list);
