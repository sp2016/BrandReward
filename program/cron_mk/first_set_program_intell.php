...
foreach($affiliate_list as $affid => $aff_v){
	...
	while(true){
		$where_str = "";
		$sql = "select a.ID, a.Name, a.CommissionExt, a.AffId,a.CategoryExt,a.CategoryFirst,a.CategorySecond,a.IdInAff, a.Homepage, a.Description, a.StatusInAff, a.Partnership, a.AffDefaultUrl, a.SupportDeepUrl, a.CommissionExt, a.TargetCountryExt, a.TargetCountryInt, a.TargetCountryIntOld, a.LastUpdateTime, b.HomepageInt, a.SecondIdInAff
				,c.ShippingCountry
				from program a left join program_int b on a.id = b.programid left join program_intell c on a.id = c.programid where a.affid = $affid $pid ";
		...
		foreach($prgm_arr as $p_v){
			...
			if(empty($shipping_country)){
				...
				if(!count($shipping_arr) && $affid != 2 && $affid != 10){
					$m = array();
				...
				if($check_name_country && $affid != 152 && $affid != 360){
					/*if($affid == 10){
						$shipping_arr = array();
					}*/
			...
			if(SID=='bdg02') {
				...
				$categoryFirstTmp = trim($p_v['CategoryFirst'],"-, \t\n\r\0\x0B");
				$categorySecondTmp = trim($p_v['CategorySecond'],"-, \t\n\r \v");
				$categoryExtTmp = trim($p_v['CategoryExt'],"-, \t\n\r \v");

				$categorySecond = $categorySecondTmp;
				$categoryFirst = $categoryFirstTmp;

				if(!$categoryFirstTmp && !$categorySecondTmp && $categoryExtTmp){
					//without CategoryFirst&CategorySecond but with CategoryExt
					$categoryFirst = $categoryExtTmp;
				}elseif(!$categoryFirstTmp && $categorySecondTmp){
					//CategoryFirst exist
					$categoryFirst = $categorySecondTmp;
				}

				$CategoryId = checkCategoryExist($categoryFirst,$categorySecond);
				//存在分类信息的入库
				if($CategoryId){
